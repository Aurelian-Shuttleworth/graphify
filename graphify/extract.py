...
def _batch_lsp_enrich_nix(
    ...
    try:
        with LspClient(nix_lsp) as client:
            ...
            def _is_nix_lockfile(path: Path) -> bool:
                """Detect autogenerated Nix lockfiles that hang LSP servers.

                Files like bun.nix (bun2nix), yarn.nix, node-packages.nix contain
                hundreds of fetchurl/fetchFromGitHub bindings in a single attrset.
                These are never useful for LSP analysis.
                """
                name = path.name.lower()
                if name in {'bun.nix', 'yarn.nix', 'node-packages.nix', 'node-composition.nix'}:
                    return True
                # Heuristic: files > 50KB that are mostly fetchurl calls
                try:
                    size = path.stat().st_size
                    if size > 50_000:
                        head = path.read_text(encoding='utf-8', errors='replace')[:2000]
                        if 'fetchurl' in head or 'fetchFromGitHub' in head:
                            return True
                except OSError:
                    pass
                return False
            ...
            for file_pos, idx in enumerate(nix_indices, 1):
                ...
                path = paths[idx]
                if _is_nix_lockfile(path):
                    try:
                        rel_path = path.relative_to(workspace_root)
                    except ValueError:
                        rel_path = path.name
                    print(
                        f"  LSP enrichment: skipped {rel_path} (autogenerated lockfile)",
                        flush=True,
                    )
                    continue
                result = per_file[idx]
